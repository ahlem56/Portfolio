...
const ContactForm = () => {
    ...
    from_name: name,        // The name from the form
    from_email: email,      // The email from the form
    message: message,       // The message from the form
    title: "Contact Us",    // A static subject or dynamic if needed
};
    ...
    try {
        ...
        // Directly insert the values here
        const mailRes = await emailjs.send(
            "service_97rh3xe",  // Direct Service ID
            "template_kwcf2q9",  // Direct Template ID
            templateParams,
            "HPiuTyQy7BQhb8yfD"  // Direct Public Key
        );

        console.log("EmailJS response:", mailRes);
